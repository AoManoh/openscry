...
func TestRetryHonorsRetryAfterHint(t *testing.T) {
	...
	opts := RetryOptions{
		MaxAttempts: 3,
		BaseDelay:   500 * time.Millisecond, // backoff would be sub-second
	...
	for i, d := range delays {
		if d != 5*time.Second {
			t.Fatalf("delay[%d]=%v want 5s (Retry-After hint should win over backoff)", i, d)
		}
	}
}

func TestRetryAfterCappedAtCeiling(t *testing.T) {
	var delays []time.Duration
	recordSleep := func(ctx context.Context, d time.Duration) error {
		delays = append(delays, d)
		return nil
	...
	opts := RetryOptions{
		MaxAttempts: 2,
		IsRetryable: retryableOnly,
		RetryAfter:  func(error) (time.Duration, bool) { return 10 * time.Minute, true }, // pathological
		sleep:       recordSleep,
	}
	_, _ = Retry(context.Background(), opts, func(context.Context) (int, error) {
		return 0, errRetryable
	})
	if len(delays) != 1 || delays[0] != maxRetryAfter {
		t.Fatalf("delays=%v want a single %v (ceiling)", delays, maxRetryAfter)
	}
}

func TestRetryAfterFalseFallsBackToBackoff(t *testing.T) {
	var delays []time.Duration
	recordSleep := func(ctx context.Context, d time.Duration) error {
		delays = append(delays, d)
		return nil
	...
	opts := RetryOptions{
		...
		Jitter:      false,
		IsRetryable: retryableOnly,
		RetryAfter:  func(error) (time.Duration, bool) { return 0, false }, // no hint
		sleep:       recordSleep,
	}
	_, _ = Retry(context.Background(), opts, func(context.Context) (int, error) {
		return 0, errRetryable
	...
	if len(delays) != 1 || delays[0] != 200*time.Millisecond {
		t.Fatalf("delays=%v want a single 200ms (backoff fallback)", delays)
	}
}
