...
./openscry version   # prints the version from build info: release tag (v0.2.2), local pseudo-version or devel
```

## IDE MCP configuration

Add openscry under `mcpServers` in your `mcp_config.json`. **Online mode is recommended** — no local build needed, `go run` pulls the source from GitHub and compiles it automatically. Pin a release tag (such as `@v0.2.2`) rather than `@main`; to upgrade, change the tag and restart the session (see [CHANGELOG.md](CHANGELOG.md)):
